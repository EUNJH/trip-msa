...
typedef struct Container {
	...
	bool isCpuScaled;
	bool isMemScaled;
	time_t cpuScaledTime;		// time when cpu scaled
	time_t memScaledTime;		// time when mem scaled
...
Con* findByName(Con* head, char* conName){
	Con* ptr;
	for(ptr=head->next; ptr!=NULL; ptr=ptr->next){
		if (strcmp(ptr->name, conName) == 0)
			return ptr;
	}
	return NULL;
}
...
void startContainer(char* conName){ // start container with name
	FILE *fp;
	string cmd = "curl -X POST -s --unix-socket /var/run/docker.sock http://v1.41/containers";
	cmd.append(conName);
	cmd.append("/start");
	fp = popen(cmd.c_str(), "r");
    pclose(fp);
...
void healthCheck(Con* head){ // check container and restart stopped container
	char conName[20];
	for (int i = 0; i < CONTAINER_NUM; i++){
		strcpy(conName, CONTAINERS[i]);
		if (findByName(head, conName) == NULL){ // if container does not exist
			startContainer(conName);
			printf("[HealthCheck] restarting container %s ...\n", conName);
		}
	}
}

void setCpu(char* name, string cpuShares){
	FILE* fp;
	string curl = "curl -X POST -s --unix-socket /var/run/docker.sock -H \"User-Agent: linux\" -H \"Content-Type: application/json\" -d \"{\\\"CpuShares\\\": ";
	string url = "}\" http://v1.41/containers";
	string endpoint = "/update";
	curl.append(cpuShares);
	curl += url;
	curl.append(name);
	curl += endpoint;
...
void setMem(char* name, string memSize){
	FILE* fp;
	string curl = "curl -X POST -s --unix-socket /var/run/docker.sock -H \
			\"User-Agent: linux\" -H \"Content-Type: application/json\" -d \"{\\\"Memory\\\": ";
	string memSwap = ", \\\"MemorySwap\\\": ";
	string url = "}\" http://v1.41/containers";
	string endpoint = "/update";
	curl += memSize + memSwap + memSize + url;
	curl.append(name);
	curl += endpoint;
	fp = popen(curl.c_str(), "r");
	pclose(fp);
...
void autoScale(Con* head){ // auto scale container cpus & mem
	Con* ptr;
	time_t now = time(NULL);
	cout << "Currnet scaling duration is " << SCALE_DURATION << "sec." << endl;
	for (ptr=head->next; ptr!=NULL; ptr=ptr->next){ // for ever container
		if (ptr->isCpuScaled){ // cpu is already scaled	
			cout << "Container " << ptr->name << "'s CPU is scaled during " << (double)(now - ptr->cpuScaledTime) << "sec." << endl;
			if ((double)(now - ptr->cpuScaledTime) > SCALE_DURATION){ // over duration
				if (65 >= ptr->per_cpu){ // is it okay now?
					printf("[Auto scaling] set %s cpus to 1 ... \n", ptr->name);
					setCpu(ptr->name, "512");
					ptr->isCpuScaled = false;
				}
			}
		} 
		else if (65 < ptr->per_cpu){ // if cpu usage is higher than 65%
			printf("[Auto scaling] set %s cpus to 2 ... \n", ptr->name);
			setCpu(ptr->name, "1024");
			ptr->isCpuScaled = true;
			ptr->cpuScaledTime = time(NULL);
		}
		if (ptr->isMemScaled){ // mem is already scaled
			cout << "Container " << ptr->name << "'s MEM is scaled during " << (double)(now - ptr->memScaledTime) << "sec." << endl;
			if ((now - ptr->memScaledTime) > SCALE_DURATION){ // over duration
				if (60 >= ptr->per_mem){ // is it okay now?
					cout << "[Auto scaling] set " << ptr->name << " mem to 1024MB ... " << endl;
					setMem(ptr->name, "1073741824");
					ptr->isMemScaled = false;
				}
			}
		} 
		else if (60 < ptr->per_mem){ // if mem usage is higher than 60%
			cout << "[Auto scaling] set " << ptr->name << " mem to 2048MB ... " << endl;
			setMem(ptr->name, "2147483648");
			ptr->isMemScaled = true;
			ptr->memScaledTime = time(NULL);
		}
	}
	return;
}
...
int main() {
	...
	while (1) {
    	...
    	for (ptr=current->next; ptr!=NULL; ptr=ptr->next) { // for each Container
    		...
    		if (cJSON_IsInvalid(csr) || csr == NULL){ // when container is being deleted
    			delAll(current);
    			goto main; // start parsing again (to prevent error when container is being deleted)
    		}
    		...
    		if (ex_info != NULL){
				...
				// to save previous scale time (for auto scaling)
				ptr->isCpuScaled = ex_info->isCpuScaled;
				ptr->cpuScaledTime = ex_info->cpuScaledTime;
				ptr->isMemScaled = ex_info->isMemScaled;
				ptr->memScaledTime = ex_info->memScaledTime;
    		}
