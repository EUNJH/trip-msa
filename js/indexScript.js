...
function geoInfo() {
  function onGeoOK(position) {
    ...
    $.ajax({
      type: "GET",
      url: `http://localhost:5084/nearspots?lat=${lat}&lng=${lng}`,
      data: {},
      success: function (response) {
        ...
        for (let i = 0; i < response.length; i++) {
          ...
          if (!file) {
            file = "http://localhost:5084/nearspots/image";
          }
      ...
      error: function (error) {
        $.ajax({
          type: "GET",
          url: `http://localhost:5094/nearspots?lat=${lat}&lng=${lng}`,
          data: {},
          success: function (response) {
            $("#near_card").empty();
            response = JSON.parse(response);
            ...
            for (let i = 0; i < response.length; i++) {
              ...
              let file = response[i]["firstimage"];
              if (!file) {
                file = "http://localhost:5094/nearspots/image";
              }
              let distance = response[i]["dist"];
              let content_id = response[i]["contentid"];
